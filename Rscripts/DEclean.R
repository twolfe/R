...
dat <- selectSpecies(data.featureCounts, " ", " ", "m","t"," ")
keep <- rowSums(cpm(dat)>0.08) >= 10 #check filtering step
summary(keep)
...
plotMD(resEdge, status=is.de, values=c(1,-1), col=c("blue","red"), legend="topright", pch=19, cex=0.5)
...
s.class(pca$li, fac=fac,
        col=c("red", "blue"),
        axesel=F, cstar=1, cpoint=1, xax = 1, yax = 2, clabel=0.9)
